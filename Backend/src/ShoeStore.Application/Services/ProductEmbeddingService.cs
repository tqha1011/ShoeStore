    ...
    IEmbeddingGenerator<string,Embedding<float>> embeddingGenerator) : IProductEmbeddingService
{
    ...
    public async Task<ErrorOr<Created>> GenerateVectorEmbeddingByProductPublicId(Guid productId, CancellationToken cancellationToken)
    {
    ...
    private async Task<List<ReadOnlyMemory<float>>> GenerateVectorEmbeddingAsync(List<string> textChunk, CancellationToken token)
    {
