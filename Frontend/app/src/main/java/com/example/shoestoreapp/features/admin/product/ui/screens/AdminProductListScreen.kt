...
fun AdminProductListScreen(
    ...
    val hasNext by viewModel.hasNext.collectAsState(initial = false)
    ...
    LaunchedEffect(lazyGridState, hasNext, isLoadingMore) {
        snapshotFlow { lazyGridState.layoutInfo.visibleItemsInfo }
            .collect { visibleItems ->
                if (visibleItems.isNotEmpty() && hasNext && !isLoadingMore) {
                    val lastVisibleIndex = visibleItems.last().index
                    val totalItems = products.size

                    if (lastVisibleIndex >= (totalItems - 2)) {
                        Log.d("Pagination Test", "Đang gọi hàm loadnextpage")
